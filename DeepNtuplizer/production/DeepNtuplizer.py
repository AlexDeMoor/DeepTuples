...
options.register('maxEvents', 100001,VarParsing.VarParsing.multiplicity.singleton,VarParsing.VarParsing.varType.int,"maximum events")
...
options.register('isttc', False, VarParsing.VarParsing.multiplicity.singleton, VarParsing.VarParsing.varType.bool, "use ttc info (gen) or not")
...
if options.isMC:
    ...
    if options.isemu or options.isdimu or options.ismutau:
        ...
        jet_coll_gen = 'selectedCJetCandidates'
    else:
...
#    process = emuSelection(process,"pfParticleNetFromMiniAODAK4PuppiCentral");
    ...
    from DeepNTuples.DeepNtuplizer.ttcharm_skim_cff import ttcharmSelection
    print ("add ttc process selection")
    process = ttcharmSelection(process,"pfUnifiedParticleTransformerAK4");
    process.deepntuplizer.leptonPairs = cms.InputTag("leptonJetPairs")
    process.deepntuplizer.jets = cms.InputTag('selectedCJetCandidates')
...
if options.isttc:
    process.deepntuplizer.ttc = cms.bool(True)
if options.isdimu:
...
elif (options.isMC and options.isttc):
    process.p = cms.Path(
        process.leptonSelection *
        process.jetSelection *
        process.deepntuplizer,
...
elif (options.isttc): #Data+Skimming
    process.p = cms.Path(
        process.leptonSelection *
        process.jetSelection *
        process.deepntuplizer,
